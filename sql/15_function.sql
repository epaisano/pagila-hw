...
$$
select category.name, count(film_id) as sum from category join film_category using (category_id) join film using (film_id) join language using (language_id) where language.name = $1 group by category.name order by name
$$
LANGUAGE SQL
IMMUTABLE
